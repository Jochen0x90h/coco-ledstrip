...
LedStrip_I2S::LedStrip_I2S(Loop_RTC0 &loop, int sckPin, int lrckPin, int dataPin, int bitTime, int resetTime)
	: loop(loop)
{
...
Awaitable<> LedStrip_I2S::stateChange(int waitFlags) {
...
void LedStrip_I2S::update() {
	auto i2s = NRF_I2S;
	switch (this->phase) {
	case Phase::COPY:
		{
			// get buffer fill size (already occupied part of the buffer)
			int size = this->size;

			// source data
			uint8_t *begin = this->data;
			uint8_t *src = begin;
			uint8_t *end2 = src + (BUFFER_SIZE - size);
			uint8_t *end = std::min(end2, this->end);

			// destination
			int offset = this->offset;
			int32_t *dst = this->buffer + offset;
			...
			if (end == end2) {
				...
				this->offset = offset ^ BUFFER_SIZE;
				this->size = 0;
				break;
			}
	...
	case Phase::FINISHED:
		{
			...
			// push finished transfer buffer to event loop so that BufferBase::handle() gets called from the event loop
			/*this->loop.push(this->transfers.pop());
...
LedStrip_I2S::BufferBase::BufferBase(uint8_t *data, int size, LedStrip_I2S &device)
	: BufferImpl(0, data, size, BufferBase::State::READY), device(device)
{
...
void LedStrip_I2S::BufferBase::startTx() {
	...
	device.resetCount = std::max(device.resetWords, BUFFER_SIZE - int(this->p.size));
