...
protected:
	LedStrip_UART_DMA(Loop_TIM2 &loop, int txPinFunction, const usart::Info &usartInfo, const dma::Info &dmaInfo,
		uint32_t brr, int resetCount);
public:
	/**
		Constructor
		@param loop event loop
		@param txPinFunction receive (RX) pin and alternative function (see data sheet)
	...
	*/
	LedStrip_UART_DMA(Loop_TIM2 &loop, int txPinFunction, const usart::Info &usartInfo, const dma::Info &dmaInfo,
		Kilohertz<> clock, Nanoseconds<> bitTime, Microseconds<> resetTime) : LedStrip_UART_DMA(loop, txPinFunction,
		//usartInfo, dmaInfo, int(clock * bitTime / 3) + 1, int(resetTime / (bitTime * 3)) + 1) {}
		usartInfo, dmaInfo, int(clock * bitTime / 3) + 1, int(clock / ((int(clock * bitTime / 3) + 1) * 9) * resetTime)) {}

	~LedStrip_UART_DMA() override;


	// internal buffer base class, derives from IntrusiveListNode for the list of buffers and Loop_TIM2::Handler2 to be notified from the event loop
	class BufferBase : public BufferImpl, public IntrusiveListNode, public Loop_TIM2::Handler2 {
	...
	*/
	void DMAx_IRQHandler() {
		...
		if ((this->dma->ISR & this->dmaFlag) != 0) {
			handle();
		}
	}
